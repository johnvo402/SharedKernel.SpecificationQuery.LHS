...
using JohnChum.SharedKernel.SpecificationQuery.LHS.Extensions.Encryption;
using JohnChum.SharedKernel.SpecificationQuery.LHS.Extensions.Expressions;
using JohnChum.SharedKernel.SpecificationQuery.LHS.Extensions.Reflections;
using Microsoft.EntityFrameworkCore;
using Newtonsoft.Json;
using static Microsoft.EntityFrameworkCore.DbLoggerCategory;

namespace JohnChum.SharedKernel.SpecificationQuery.LHS.Extensions.QueryExtensions;
...
public static class PaginationExtension
{
    ...
    public static PaginationResponse<T> ToPagedList<T>(
        ...
        int current,
        int size
    ) => new(query.Skip((current - 1) * size).Take(size), (query.Count() > size) ? (int)Math.Ceiling((query.Count() / (double)size)) : 1, current, size);
    ...
    )
    {
        ...
        if (totalCount == 0)
        {
            return new PaginationResponse<T>(query, 1, request.Size);
        }
        int totalPage = totalCount > request.Size ? (int)Math.Ceiling(totalCount / (double)request.Size) : 1;
 ...
 )
    {
        ...
        // Xử lý Enum
        if ((memberType.IsNullable() && memberType.GenericTypeArguments.Length > 0 && memberType.GenericTypeArguments[0].IsEnum) || memberType.IsEnum)
        {
        ...
        if (memberType == typeof(DateTimeOffset) || memberType == typeof(DateTimeOffset?))
        {
            ...
            {
                string stringValue => DateTimeOffset.TryParse(stringValue, CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal, out var parsedDate) ? parsedDate : null,
            ...
            {
                throw new InvalidOperationException($"Cannot convert value '{value}' to {memberType}.");
            }
        ...
        if (memberType.IsNullable())
        {
            ...
            object? convertedValue = value != null ? Convert.ChangeType(value, underlyingType) : null;
        ...
        {
            return new(member, Expression.Constant(Convert.ChangeType(value, memberType), memberType));
        }
